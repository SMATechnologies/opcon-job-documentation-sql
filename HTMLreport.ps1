param(
    ...
    $password,
    $port = 587,
    $subject = "OpCon Documentation Report",
    $body
)
...
While ($reader.Read())
{
    #Gets information for the button
...
$Connection.Dispose()
#-------------------------------------------------------
$schedulesArray = @($schedules | Select -Unique) | Sort-Object
...
# Uncomment the below lines to attach the report as an email, could also be done inside of OpCon
#$secpasswd = ConvertTo-SecureString "$password" -AsPlainText -Force
#$mycreds = New-Object System.Management.Automation.PSCredential ($user, $secpasswd)
$body | Out-File $path

#Send-MailMessage -SmtpServer $smtp -To $to -From $user -Subject $subject -Body "See attachment for report" -Attachments $path  -BodyAsHtml -UseSSL -Credential $mycreds -Port $port
#Remove-Item $path